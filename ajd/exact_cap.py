                ...
                if i == a:
                    ...
                    if test_factor_1 and test_factor_2 and test_factor_3:
                        ...
                        if stopping_time < T:
                            ...
                            while cap_limits[cap_counter] < stopping_time:
                    ...
                    if test_factor_1 and test_factor_2 and test_factor_3:
                        ...
                        for jump in exponential_times:
                            if candidate_times[i] < jump:
                                break
                            for t_cap in cap_times:
                                # check if we jump over any of cap strike times
                                if current_t < t + t_cap < t + jump:
                                    w = candidate_bridge[t_cap]
                                    cap_value += g * max(f_inv(y + w) - strike, 0) * np.exp(-x_down * t_cap)
                                    # print('cap added @ t=%s' % (t + t_cap))
                        ...
                        if cap_counter < 2:
                            while cap_limits[cap_counter] < stopping_time and cap_counter < 2:
                                t_cap = cap_limits[cap_counter] - t
                                w = candidate_bridge[t_cap]
                                cap_value += g * max(f_inv(y + w) - strike, 0) * np.exp(-x_down * t_cap)
                                # print('cap added @ t=%s' % (t + t_cap))
...
# print(monte_carlo(n_sim=500000, sim_frames=[1000, 2000, 5000, 10000, 50000, 100000, 500000]))
