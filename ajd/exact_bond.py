    ...
    while sim_count < n_sim:
        ...
        while t < T:
            ...
            while rejected:
                ...
                all_times = sorted(candidate_times + test_times + exponential_times)
                if T - t < tau:
                ...
                if i == a:
                    ...
                    K_max = 1.382436647017447  # previously computed location of global maximum
        ...
        if sim_count in sim_frames:
            print(sim_count)
            bond_price = np.mean(bond_results)
            sample_std = np.std(bond_results)
            std_error = sample_std / np.sqrt(sim_count)
            time_spent = time.time() - start_time
            sim_data.append([sim_count, bond_price, std_error, time_spent])

    return sim_data


# print(monte_carlo(n_sim=100000, sim_frames=[1000, 2000, 5000, 10000, 50000, 100000]))
