...
def monte_carlo(n_sim=1000, sim_frames=[], T=1, strike=65, X_0=50, beta=-1, r=0.05,
                sigma=50/4, b=0, c=1/2, down_barrier=5):
        ...
        while t < T:
            ...
            while rejected:
                    ...
                    for i in range(len(y_range)):
                        if y_range[i] < y:
                            theta_opt = (optimal_thetas[i] * (y_range[i + 1] - y) + optimal_thetas[i + 1] * (
                                    y - y_range[i])) / (y_range[i + 1] - y_range[i])
                            theta = min(theta_opt, y-f(down_barrier))
                            break
                else:
                    theta = min(1.5, (y - X_0 ** (-beta) / (sigma * beta)) / 2, y-f(down_barrier))
                ...
                if i == a:
                    ...
                    test_factor_1 = (np.log(u) <= integrated_drift(y + candidate_bridge[min(tau, T-t)])
                    ...
                    if test_factor_1 and test_factor_2 and test_factor_3:
                        ...
                        if stopping_time < T:
                            ...
                            if y < f(down_barrier)+1e-7:
                                # the barrier has been hit
                                option_results.append(0)
                                hitting_times.append(t)
                                t = T  # to break the loop

                        else:
                            y += candidate_bridge[T-t]
                            final_samples.append(f_inv(y))
                            option_results.append(max(f_inv(y)-strike, 0))
                            t = T
                    ...
                    u, v = np.random.rand(2)
                    K = np.exp(integrated_drift(y+theta))
                    S = max(np.exp(-phi_min*min(tau, (T-t))), 1)

                    # first Bernoulli variable
                    test_factor_1 = (u*K <= np.exp(integrated_drift(y + candidate_bridge[candidate_times[i]])))
                    # second Bernoulli variable
